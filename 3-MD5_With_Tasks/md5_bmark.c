...
void run(md5bench_t* args) {
    ...
    #pragma omp single nowait
    {
		for(int i = 0; i < args->iterations; i++) {
			buffers_to_process = args->numinputs;
			next = 0;
			uint8_t** input = args->inputs;
			uint8_t* output = args->out;
			
			while (buffers_to_process > 0) {			
				
				#pragma omp task firstprivate(next) \
					depend(out:output[next * DIGEST_SIZE]) \
					depend(out:input[next])
				{
					process(input[next], &output[next * DIGEST_SIZE], args->size); 
				}
				next++;
				buffers_to_process--;
			}
